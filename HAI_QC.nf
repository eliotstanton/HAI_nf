...
process filtering {
	...
	output:
	tuple name, file("${name}_filtered{_R1,_R2}.fastq.gz") into read_files_spades, read_files_kraken2, read_files_quast, filtered_files_deliverables
...
process kraken2 {
	...
	output:
	tuple name, file("${name}_kraken2_report.txt") into kraken_files_deliverables
...
process spades {
	...
	output:
	tuple name, file("${name}.fa") into assembled_genome_mlst, assembled_genome_quast, assembled_genome_contigs
...
process contigs {
	...
	input:
	set val(name), file(reads) from assembled_genome_contigs
...
process quast {
	...
	output:
	tuple name, file("./quast/report.txt") into results_quast
...
process deliverables {
	...
	input:
	set val(name4), file(reads) from read_files_deliverables
	set val(name2), file(results) from kraken_files_deliverables
	set val(name3), file(assembly) from results_quast
	set val(name), file(reads) from filtered_files_deliverables

	output:
	tuple name, file("${name}_stats.txt") into results_deliverables
...
file_FASTQ1     = "${name}_filtered_R1.fastq"
file_FASTQ2     = "${name4}_R1.fastq.gz"
